...
**(2) 放电源符号与标志**：
- 按快捷键 **`P`**（或点击右侧 **Add a power port / 添加电源端口** 按钮，或者按 `A` 搜 `power` 库）。
- 放置电源与地：搜 `+5V` 放一个，搜 `GND` 放一个。
- **关键必做（避坑）**：放置两个 **`PWR_FLAG`**（电源标志符号），一个用导线连到 `+5V` 网络，一个连到 `GND` 网络。（KiCad 规定外部接插件进电必须贴此标志，否则 ERC 会报 `Input Power pin not driven` 错误）。

**(3) 连线**：按 `W`（或点击右侧 Wire 工具），把回路接成闭合环路：
...
> 🚨 **新手必踩经典报错排查**：
> - **报错 `Error: Input Power pin not driven by any Output Power pins`**：
>   - **原因**：KiCad 规定受电端必须有电源源头声明。排针属于被动元件，若没标明从何处供电，ERC 会警告电源脚悬空。
>   - **解法**：在 `+5V` 和 `GND` 导线上各连一个 **`PWR_FLAG`** 符号（见步骤 2），再跑一次即 0 错误通过！
>
...
2. 将回路中的供电线（如 `+5V` 到电阻，电阻到 LED 阳极）连接完毕。
3. 如果一条线要换层（顶层↔底层），布线过程中按 `V` 放一个**过孔**（立交桥，让线钻到板子另一面）。
...
#### (2) 大面积铺铜（Copper Pour / Zone）

> 🌊 **为什么要铺铜？通俗理解「自来水管与下水大海」**：
> - **布线就像拉自来水管**：一根根细吸管把电精准送到某个特定器件。
> - **铺铜就像在脚下建一片「大海」**：
>   1. **所有 GND 引脚就地入海**：如果板子上有几十个接地引脚，不用挨个拉长长的细线回到电源负极，直接就近打孔连到脚下整片铜皮（大海）即可，阻抗极低、地电位极其平稳，布线效率提升数倍！
>   2. **抗干扰金钟罩**：整片连着 GND 的大铜皮宛如一个金属屏蔽盒，能吸收阻隔外界电磁辐射杂波（以后的 Wi-Fi、蓝牙、音频设计绝对必备）。
>   3. **符合板厂工艺**：PCB 制造原材料本来就是整面覆铜的，板厂是用化学药水把没用的铜“腐蚀啃掉”。若不铺铜，95% 的空白铜都要被药水啃掉，容易造成板子受热翘曲变形；铺铜保留大面积铜皮，既省药水又结实平整耐热。

**铺铜四步操作法**：
1. **激活工具**：按快捷键 `Ctrl+Shift+Z`（Mac 上为 `Cmd+Shift+Z`，或点击右侧工具栏的“斜纹填充区”图标）。
2. **选择网络（核心）**：在板框内空白处点一下，在弹出的属性窗口中：
   - 勾选图层：`F.Cu`（顶层）或 `B.Cu`（底层，初学双面板建议顶底两层都铺）。
   - **选中网络：`GND`**（告诉软件这片铜皮通地线）➔ 点确定。
3. **画铺铜边框**：顺着板框外侧依次点击 4 个角闭合成一个多边形框，最后双击完成。
4. **注入铜水（按 `B`）**：在键盘上敲击字母 **`B`**（Build/重新填充）。
   - 💥 整块板子空白处立刻铺满铜皮，所有属于 `GND` 的焊盘会自动长出十字花焊盘与大海连通，GND 飞线彻底消失！
